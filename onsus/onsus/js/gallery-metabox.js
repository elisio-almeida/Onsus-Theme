jQuery(function($) {
    $('body').on('click', '.tf_multi_upload_image_button', function (e) {
        e.preventDefault();
        ...
        custom_uploader.on('select', function () { 
          ...
          $(button).siblings('.tf_multi_remove_image_button').show();
        ...
        custom_uploader.open();
           ...
           * Remove image event
           */
      $('body').on('click', '.tf_multi_remove_image_button', function () {
        $(this).hide().prev().val('').prev().addClass('button').html('Add  Media');
        $(this).parent().find('ul').empty();
        return false;
      });
    
      jQuery(document).on('click', '.tf-multi-upload-medias ul li i.delete-img', function () {
        var ids = [];
        var attach_id =  jQuery(this).parents('li').attr('data-attechment-id');
        jQuery('.tf-multi-upload-medias ul li').each(function () {
          if( attach_id != jQuery(this).attr('data-attechment-id') ){
            ids.push(jQuery(this).attr('data-attechment-id'));  
          }
    
        });
        jQuery(this).parents('.tf-multi-upload-medias').find('input[type="hidden"]').val(ids);
